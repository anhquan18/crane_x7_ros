...
private:
  ...
  void offset_direction_init_pose()
  {
    std::vector<double> joint_values;
    joint_values.push_back(angles::from_degrees(-40.0));
  ...
  void picking(tf2::Vector3 target_position)
  {
    ...
    // Trajectoryがうまく生成されるための掴む準備姿勢
    control_arm(target_position.x(), target_position.y(), target_position.z() + 0.25, 0, 180, 0);
    control_arm(target_position.x(), target_position.y(), target_position.z() + 0.07, 0, 180, 0);

    /*
    //物体の真上まで移動
    target_pose.position.x = target_position.x();
    target_pose.position.y = target_position.y();
    target_pose.position.z = target_position.z()+0.25;
    waypoints.push_back(target_pose);

    //物体を掴みに行く
    target_pose.position.x = target_position.x();
    target_pose.position.y = target_position.y();
    target_pose.position.z = target_position.z()+0.07;
    waypoints.push_back(target_pose);

    //軌道生成
    move_group_arm_->computeCartesianPath(
