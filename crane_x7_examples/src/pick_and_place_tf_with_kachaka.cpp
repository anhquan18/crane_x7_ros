...
private:
  void on_timer()
  {
    // picking_idのtf位置姿勢を取得
    geometry_msgs::msg::TransformStamped tf_msg;

    // テーブルでの箱をpickingしないように
    if (current_picking_id == table_box_id)
	    current_picking_id++;

    // ID:5までしかpickingできないように設定する
    if (current_picking_id == 6)
	    current_picking_id = 0;

    try {
      tf_msg = tf_buffer_->lookupTransform(
        "base_link", "target_" + std::to_string(current_picking_id),
        tf2::TimePointZero);
  ...
  void table_direction_init_pose()
  {
  ...
  void kachaka_direction_init_pose()
  {
    std::vector<double> joint_values;
    joint_values.push_back(angles::from_degrees(-90.0));
    joint_values.push_back(angles::from_degrees(90));
    joint_values.push_back(angles::from_degrees(0.0));
    joint_values.push_back(angles::from_degrees(-160));
    joint_values.push_back(angles::from_degrees(0.0));
  ...
  void picking(tf2::Vector3 target_position)
  {
    ...
    if (current_direction == "kachaka") {
      ...
      // 移動する
	    ...
	    // 下ろす
	    control_arm(0.25 + placing_flag, 0.0, 0.13, -180, 0, 90);
	    ...
	    // カメラをtableの方に向かせる
	    table_direction_init_pose();
	    current_direction = "table";

	    //  picking終了した場合、間違ってテーブルの箱を移動させないようにIDを登録する
	    table_box_id = current_picking_id;
